...
std::ostream &operator<<(std::ostream &strm, TokenType tt) {
  const std::string tokenNames[] = {
      ...
      "If",     "Else",    "LessThan",   "GreaterThan", "For",
      "While",  "Class",   "Dot",        "New"};
  ...
  std::vector<std::shared_ptr<Token>> lex(std::string input) {
    ...
    while (it < input.end()) {
      if (std::isspace(*it)) {
        ...
        auto token =
            std::make_shared<Token>(Token(TokenType::String, stringValue));
        tokens.emplace_back(token);
        it++;
      } else if (*it == '(') {
        ...
        auto token = std::make_shared<Token>(Token(TokenType::Semicolon));
        tokens.emplace_back(token);
        it++;
      } else if (*it == '+') {
        ...
        auto token = std::make_shared<Token>(TokenType::Dot);
        tokens.emplace_back(token);
        it++;
      } else {
        std::string identifierValue;
        ...
        } else if (identifierValue == "class") {
          token = std::make_shared<Token>(TokenType::Class);
        } else if (identifierValue == "new") {
          token = std::make_shared<Token>(TokenType::New);
        } else {
